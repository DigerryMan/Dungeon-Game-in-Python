...
class RoomGenerator:
    ...
    def generate_room(self, entry_direction: Directions):
        ...
        while not self.well_generated:
            ...
            room_map = [
                [self.room.layout[y][x] for x in range(len(self.room.layout[y]))]
                for y in range(len(self.room.layout))
            ]
            ...
            for y, row in enumerate(self.room.layout):
                for x, col in enumerate(row):
                    if (
                        col == "C"
                        and not self.room.chest
                        and random.uniform(0, 1) < 0.2 + self.game.player.get_luck()
                    ):
                        ...
                        self.room.shop_stands.append(
                            ShopStand(self.game, x + 0.5, y + 0.5)
                        )
                        ...
                        if (
                            self.room_type not in special_rooms
                            and col != "#"
                            and random.uniform(0, 1) < random_block_density_factor
                        ):  # chance of random block that wasn't planned
                            if random.uniform(0, 1) < 0.5:
                                self.room.blocks.append(
                                    DestructableBlock(self.game, x, y)
                                )
                                room_map[y][x] = "D"
                            else:
                                self.room.blocks.append(Block(self.game, x, y))
            ...
            for y, x in doors_positions:
                ...
                if self.room_type == "boss":
                    self.room.doors.append(
                        Door(self.game, x, y, direction, self.level, "boss_door")
                    )
                else:
                    current_position = self.game.map.current_position
                    neighbor_room = self.game.map.room_map[
                        current_position[0] + direction.value[0]
                    ][current_position[1] + direction.value[1]]
                    if neighbor_room.room_type == "boss":
                        ...
                        if self.room_type == "shop":
                            self.room.doors.append(
                                Door(
                                    self.game, x, y, direction, self.level, "shop_door"
                                )
                            )
                        else:
                            self.room.doors.append(
                                Door(self.game, x, y, direction, self.level)
                            )
            ...
            for door in self.room.doors:
                if (
                    door.direction == entry_direction.reverse()
                ):  # if the door is the one the player used to enter the room
    ...
    def get_doors_positions(self):
        ...
        for i in range(len(self.doors_to_spawn)):
            if self.doors_to_spawn[i] == Directions.UP:
                ...
                self.room.crucial_positions.append(
                    (1, self.game.settings.MAP_WIDTH // 2 - 1)
                )
                self.room.crucial_positions.append(
                    (1, self.game.settings.MAP_WIDTH // 2)
                )

            elif self.doors_to_spawn[i] == Directions.DOWN:
                doors_positions.append(
                    (
                        self.game.settings.MAP_HEIGHT - 1,
                        self.game.settings.MAP_WIDTH / 2 - 0.5,
                    )
                )
                self.room.crucial_positions.append(
                    (
                        self.game.settings.MAP_HEIGHT - 2,
                        self.game.settings.MAP_WIDTH // 2 - 1,
                    )
                )
                self.room.crucial_positions.append(
                    (
                ...
                self.room.crucial_positions.append(
                    (self.game.settings.MAP_HEIGHT // 2, 1)
                )

            elif self.doors_to_spawn[i] == Directions.RIGHT:
                doors_positions.append(
                    (
                        self.game.settings.MAP_HEIGHT / 2 - 0.5,
                        self.game.settings.MAP_WIDTH - 1,
                    )
                )
                self.room.crucial_positions.append(
                    (
    ...
    def spawn_outer_walls(self, doors_positions):
        ...
        while x < self.game.settings.MAP_WIDTH:
            if (0, x + 0.5) not in doors_positions:
                ...
                self.room.door_blockers.append(Wall(self.game, x + 0.5, 0, True))
        ...
        while x < self.game.settings.MAP_WIDTH:
            if (self.game.settings.MAP_HEIGHT - 1, x + 0.5) not in doors_positions:
                ...
                self.room.door_blockers.append(
                    Wall(self.game, x + 0.5, self.game.settings.MAP_HEIGHT - 1, True)
                )
                self.room.walls.append(
                    Wall(self.game, x - 0.5, self.game.settings.MAP_HEIGHT - 1)
                )
                self.room.walls.append(
                    Wall(self.game, x + 1.5, self.game.settings.MAP_HEIGHT - 1)
                )
                x += 1
        ...
        while y < self.game.settings.MAP_HEIGHT:
            if (y, 0) not in doors_positions:
                ...
                self.room.door_blockers.append(Wall(self.game, 0, y, True))
        ...
        while y < self.game.settings.MAP_HEIGHT:
            if (y, self.game.settings.MAP_WIDTH - 1) not in doors_positions:
                ...
                self.room.door_blockers.append(
                    Wall(self.game, self.game.settings.MAP_WIDTH - 1, y, True)
                )
    ...
    def check_if_room_well_generated(self, room_map):
        ...
        while len(q) > 0:
            ...
            for i in range(4):
                ...
                if (
                    new_row > 0
                    and new_row < height - 1
                    and new_col > 0
                    and new_col < width - 1
                    and not visited[new_row][new_col]
                    and room_map[new_row][new_col] not in ["B", "C"]
                ):
