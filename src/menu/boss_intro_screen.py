...
class BossIntroScreen:
    ...
    def display(self, boss):
        ...
        boss_spot_position = (
            self.game.settings.WIN_WIDTH // 2.3,
            self.game.settings.WIN_HEIGHT // 1.7,
        )

        player_spot = self.game.image_loader.boss_intro["playerspot"]
        player_spot_position = (
            self.game.settings.WIN_WIDTH // 20,
            self.game.settings.WIN_HEIGHT // 1.4,
        )

        boss_image = boss.animation.intro_image
        boss_image_position = (
            boss_spot_position[0]
            + boss_spot.get_width() // 2
            - boss_image.get_width() // 2,
            boss_spot_position[1]
            + boss_spot.get_height() // 2
            - boss_image.get_height(),
        )

        player_image = self.game.player.animation.intro_image
        player_image_position = (
            player_spot_position[0]
            + player_spot.get_width() // 2
            - player_image.get_width() // 2,
            player_spot_position[1]
            + player_spot.get_height() // 2
            - player_image.get_height() // 1.5,
        )
        ...
        player_image_start_position = (
            player_spot_start_position[0]
            + player_spot.get_width() // 2
            - player_image.get_width() // 2,
            player_spot_start_position[1]
            + player_spot.get_height() // 2
            - player_image.get_height() // 1.5,
        )
        boss_image_start_position = (
            boss_spot_start_position[0]
            + boss_spot.get_width() // 2
            - boss_image.get_width() // 2,
            boss_spot_start_position[1]
            + boss_spot.get_height() // 2
            - boss_image.get_height(),
        )
        ...
        while should_display:
            ...
            player_spot_current_position = (
                player_spot_start_position[0]
                + (player_spot_position[0] - player_spot_start_position[0]) * progress,
                player_spot_position[1],
            )
            boss_spot_current_position = (
                boss_spot_start_position[0]
                + (boss_spot_position[0] - boss_spot_start_position[0]) * progress,
                boss_spot_position[1],
            )

            player_image_current_position = (
                player_image_start_position[0]
                + (player_image_position[0] - player_image_start_position[0])
                * progress,
                player_image_position[1],
            )
            boss_image_current_position = (
                boss_image_start_position[0]
                + (boss_image_position[0] - boss_image_start_position[0]) * progress,
            ...
            self.display_names(
                self.game.player.animation.intro_name, vs, boss.animation.intro_name
            )
    ...
    def display_names(self, player_name, vs, boss_name):
        vs_position = (
            ...
            self.game.settings.WIN_HEIGHT // 5,
        )
        player_name_position = (
            vs_position[0] + vs.get_width() // 2 - player_name.get_width() // 2,
            vs_position[1] - player_name.get_height(),
        )
        boss_name_position = (
            vs_position[0] + vs.get_width() // 2 - boss_name.get_width() // 2,
            vs_position[1] + vs.get_height(),
        )
