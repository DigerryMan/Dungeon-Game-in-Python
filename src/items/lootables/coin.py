...
class Coin(LootableItem):
    ...
    def picked_up(self):
        ...
        self.clean_up()
        self.game.sound_manager.play_if_not_playing("coinPickup")
        return ItemType.COIN, self.value

    def update(self):
        super().update()
        self.shine_timer = (
            self.shine_timer - 1 if self.shine_timer > 0 else self.shine_interval
        )
        if self.drop_animation_time > 0:
            if self.drop_animation_time == 5:
                self.game.sound_manager.play_if_not_playing(
                    f"coinDrop{random.randint(1, 2)}"
                )
            if self.drop_animation_time % self.time_per_frame_drop == 0:
                self.drop_animation_frame = (self.drop_animation_frame + 1) % 8
                self.image = self.game.image_loader.lootables[
                    f"{self.type}_coin_drop{self.drop_animation_frame}"
                ].copy()
        ...
        if not self.is_picked_up and self.shine_timer < 6 * self.time_per_frame_shine:
            if self.shine_timer % self.time_per_frame_shine == 0:
                ...
                self.image = self.game.image_loader.lootables[
                    f"{self.type}_coin_shine{self.shine_animation_frame}"
                ].copy()

        if self.is_picked_up:
            if (
                self.pickup_timer > 0
                and self.pickup_timer % self.time_per_frame_pickup == 0
            ):
                self.image = self.game.image_loader.lootables[
                    f"gold_coin_pickup{self.pickup_animation_frame}"
                ].copy()
