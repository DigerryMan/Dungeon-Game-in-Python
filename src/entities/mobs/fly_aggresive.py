...
class FlyAggresive(Fly):
    ...
    def prepare_images_v2(self):
        self.images[0] = self.img.subsurface(
            pygame.Rect(
                1 * self.MOB_SIZE, 1 * self.MOB_SIZE, self.MOB_SIZE, self.MOB_SIZE
            )
        )
        self.images[1] = self.img.subsurface(
            pygame.Rect(
                2 * self.MOB_SIZE, 1 * self.MOB_SIZE, self.MOB_SIZE, self.MOB_SIZE
            )
        )
    ...
    def collide_blocks(self, orientation: str):
        rect_hits_collidables = pygame.sprite.spritecollide(
            self, self.game.collidables, False
        )
