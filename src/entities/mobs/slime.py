...
class Slime(Enemy):
    def __init__(self, game, x: int, y: int):
        ...
        self.possible_jumps = [
            (x_i, y_j)
            for x_i in range(-self.jump_range, self.jump_range + 1)
    ...
    def jump(self):
        ...
        if self.jump_time_left <= 0:  # end of jump
            ...
            # FOR SHADOW DISPLAY
            self.shadow_y = int(
                (self.old_jump_y + self.v_shadow_y * elapsed_time_sec)
    ...
    def calculate_parabolic_jump(self):
        ...
        self.tg = ((self.new_jump_y - self.old_jump_y) - 0.5 * 9.81 * (self.t) ** 2) / (
            self.v_x * (self.t)
        )
    ...
    def is_valid_move(self, x, y):
        if (
            x <= 0
            or x >= self.game.settings.MAP_WIDTH - 1
            or y <= 0
            or y >= self.game.settings.MAP_HEIGHT - 1
        ):
            return False
        ...
        if not self.is_jumping:
            ...
            if self.next_jump_time_left > int(0.7 * self.jump_cd):  # landing
                ...
                if self.next_jump_time_left in self.animation_jump_time:
                    if (
                        self.next_jump_time_left == self.animation_jump_time[-1]
                        and self.is_small_change_of_x
                    ):
            ...
            if self.is_change_of_frame:
                ...
                if self.is_jumping_left and self.next_jump_time_left < int(
                    0.3 * self.jump_cd
                ):
