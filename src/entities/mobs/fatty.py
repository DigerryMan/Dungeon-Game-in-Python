...
class Fatty(Wanderer):
    ...
    def __prepare_images(self):
        ...
        w, h = 41, 41
        for x in range(8):
            img = self.img.subsurface(pygame.Rect(x * 64 + x_offset, y * 64 + y_offset, w, h))
            self.images_legs.append(pygame.transform.scale(img, (self.MOB_SIZE, self.MOB_SIZE)))
        y=3 
        for x in range(4):
            img = self.img.subsurface(pygame.Rect(x * 64 + x_offset, y * 64 + y_offset, w, h))
            self.images_legs.append(pygame.transform.scale(img, (self.MOB_SIZE, self.MOB_SIZE)))
        ...
        for x in range(6):
            ...
            self.images_head.append(pygame.transform.scale(img, (int(self.MOB_SIZE*0.8), int(0.8*self.MOB_SIZE))))

    def next_frame(self):
        legs_frame = self.set_up_legs_frame() 
        self.set_up_head_frame()
        
        self.frame = pygame.Surface((self.MOB_SIZE, self.MOB_SIZE * 1.1), pygame.SRCALPHA)
