...
class Bullet(pygame.sprite.Sprite):
    ...
    def collide_blocks(self):
        ...
        if block_hits:
            mask_block_hits = [
                ...
                if self.get_mask_colliding_sprite([block_hit])
            ]
            if mask_block_hits:
                self.is_alive = False
    ...
    def animate_and_destroy(self):
        if self.animation_time == 30:
            self.game.sound_manager.play("tearPop")
            self.rect.x -= self.BULLET_SIZE
            self.rect.y -= self.BULLET_SIZE
            self.image = self.game.image_loader.tears[
                self.color + "_tear_pop" + str(self.frame)
            ].copy()
