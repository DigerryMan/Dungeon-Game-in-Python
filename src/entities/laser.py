...
class Laser(pygame.sprite.Sprite):
    ...
    def __init__(
        ...
        self.rect = pygame.Rect(
            self.calculate_rect_cords(x, y), (self.width, self.height)
        )
    ...
    def calculate_rect_cords(self, x, y):
        ...
        last_x_possible = self.game.settings.TILE_SIZE * (
            self.game.settings.MAP_WIDTH - 1
        )
        first_y_possible = self.game.settings.TILE_SIZE
        last_y_possible = (
            self.game.settings.TILE_SIZE * (self.game.settings.MAP_HEIGHT - 1)
    ...
    def collide(self):
        if self.is_friendly:
            ...
            if mob_hits:
                mobs_to_get_hit = [
                    mob_hit
                    for mob_hit in mob_hits
                    if self.get_mask_colliding_sprite([mob_hit])
                ]
            ...
            player_hits = pygame.sprite.spritecollide(
                self, self.game.player_sprite, False
            )
            if player_hits:
