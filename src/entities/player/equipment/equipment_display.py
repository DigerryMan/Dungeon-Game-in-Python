...
from config import (
    BASE_BULLET_FLY_TIME,
    BASE_IMMORTALITY_AFTER_HIT,
    BASE_SHOOTING_COOLDOWN,
    BASE_SHOT_SPEED,
)
...
class EquipmentDisplay:
    def __init__(self, equipment, game):
        ...
        self.items_x = [
            ...
            for i in range(
                self.first_item_x,
                self.first_item_x + self.item_distance * self.item_in_row,
                self.item_distance,
            )
        ]
        self.items_y = [
            ...
            for i in range(
                self.first_item_y,
                self.first_item_y + self.item_distance * self.item_in_col,
                self.item_distance,
            )
        ]
        ...
        self.highlighted_item_index = (
            self.highlighted_item_col * self.item_in_row + self.highlighted_item_row
        )
    ...
    def draw_items(self, screen):
        ...
        for item in self.eq.items:
            ...
            item_image = pygame.transform.scale(
                item["image"], (self.item_size, self.item_size)
            )
        ...
        if "description" in item["stats"].keys():
            ...
            for i, line in enumerate(description_lines):
                ...
                description_rect = description_text.get_rect(
                    center=(x, y + i * self.big_item_size // 4)
                )
            ...
            for key, value in item["stats"].items():
                ...
                stat = font_2.render(
                    key.replace("_", " ") + f": {char}{value}", True, self.font_color
                )
        ...
        if event_key is not None:
            if event_key == pygame.K_DOWN or event_key == pygame.K_s:
                ...
                self.game.sound_manager.play(
                    f"select{'Right' if random.randint(0, 1) else 'Left'}"
                )
    ...
    def set_cursor_and_big_item(self):
        ...
        self.cursor_y = (
            self.items_y[self.highlighted_item_col]
            - self.cursor_size
        ...
        item, size = self.eq.items[self.highlighted_item_index]["image"], (
            self.big_item_size,
            self.big_item_size,
        )
