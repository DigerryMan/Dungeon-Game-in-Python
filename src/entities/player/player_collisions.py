...
class PlayerCollisionEngine:
    ...
    def collide_blocks(self, direction_axis: str):
        rect_hits = pygame.sprite.spritecollide(
            self.player, self.game.collidables, False
        )
        if rect_hits:
    ...
    def check_items_pick_up(self):
        ...
        if rect_hits:
            items = [
                self.get_mask_colliding_sprite([rect_hit]) for rect_hit in rect_hits
            ]
            for item in items:
                    ...
                    match type:
                        ...
                        case ItemType.ITEM:
                            ...
                            item_image = pygame.transform.scale(
                                item_info["image"],
                                (
                                    self.game.settings.TILE_SIZE // 1.3,
                                    self.game.settings.TILE_SIZE // 1.3,
                                ),
                            )
                            self.player.animation.prepare_item_pick_up_animation(
                                item_image
                            )
                        case ItemType.PILL:
