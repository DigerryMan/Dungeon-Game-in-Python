        ...
        if self.shot_try or self.shoot_second_bullet >= 0:
            if self.shot_time_left <= 0:
                ...
                if self.player.eq.extra_stats["extra_shot_time"]:
                    self.shoot_second_bullet = self.player.eq.extra_stats[
                        "extra_shot_time"
                    ]
    ...
    def shoot_one_bullet(self, x_y_vel):
        ...
        if PLAYER_SHOOT_DIAGONAL:
            ...
            if x_y_vel[other_axis_index]:
                additional_v = int(
                    self.player.get_shot_speed()
                    * x_y_vel[other_axis_index]
                    * DIAGONAL_MULTIPLIER
                )
        ...
        Bullet(
            ...
            self.player.get_shot_speed(),
            True,
            (self.player.dmg + self.player.eq.stats["dmg"])
            * self.player.eq.extra_stats["dmg_multiplier"],
            BASE_BULLET_FLY_TIME + self.player.eq.stats["bullet_fly_time"],
