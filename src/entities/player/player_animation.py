...
class PlayerAnimation:
    ...
    def prepare_images(self):
        ...
        for x in range(6):
            self.head_images.append(
                self.img.subsurface(
                    pygame.Rect(
                        x * self.PLAYER_SIZE, 0, self.PLAYER_SIZE, self.PLAYER_SIZE
                    )
                )
            )
    ...
    def prepare_intro_images(self):
        img = self.game.image_loader.images_dict[f"{self.player_type.value}_display"][
            "boss_intro"
        ]["image"]
        ...
        img = self.game.image_loader.player_animations_list[
            self.player_type.get_index()
        ]["like0"]
    ...
    def item_pick_up_animate(self):
        ...
        if self.item_pick_up_time <= 0:
            ...
            self.item_picked_up_index = (1 + self.item_picked_up_index) % len(
                self.item_frames
            )
    ...
    def check_tear_animation(self):
        self.head_tear_anime_time_left -= 1
        if (
            self.head_tear_anime_time_left == self.head_tear_anime_cd - 1
            or self.head_tear_anime_time_left == 1
        ):
    ...
    def next_frame(self):
        ...
        self.frame.blit(
            self.head_frame, ((self.PLAYER_SIZE - self.head_frame.get_width()) // 2, -3)
        )
        if self.player.immortality_time_left > 1:
            self.frame = ImageTransformer.change_opacity(
                self.frame, self.calculate_opacity()
            )
    ...
    def set_body_frame(self):
        if (
            ...
            or self.player.direction == Directions.RIGHT
        ):
    ...
    def set_head_frame(self):
        ...
        if (
            self.player.facing == Directions.LEFT
            or self.player.facing == Directions.RIGHT
        ):
            x += 2
        ...
        self.head_frame = pygame.transform.scale(
            self.head_frame, (self.PLAYER_SIZE * 0.9, self.PLAYER_SIZE * 0.9)
        )
    ...
    def set_player_animation_image(self, frame_name: str):
        self.player.image = self.game.image_loader.player_animations_list[
            self.player_type.get_index()
        ][frame_name]
    ...
    def correct_player_mask(self, mask):
        removed_hitbox_from_sides = pygame.Surface(
            (mask.get_size()[0] * 0.25, mask.get_size()[1])
        )
        removed_hitbox_from_top = pygame.Surface(
            (mask.get_size()[0], mask.get_size()[1] * 0.25)
        )
        ...
        mask.erase(
            cut_mask_sides, (mask.get_size()[0] - cut_mask_sides.get_size()[0], 0)
        )
    ...
    def draw_additional_images(self, screen):
        if self.item_picked_up:
            blit_position = (
                self.player.rect.centerx - self.picked_up_item_image.get_width() // 2,
                self.player.rect.y - self.picked_up_item_image.get_height() // 1.5,
            )
