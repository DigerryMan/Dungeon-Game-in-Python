...
class StatBars:
    def __init__(self, game, player):
        ...
        self.font = pygame.font.Font(
            self.font_path, int(self.STAT_BARS_HEALTH_SIZE + self.font_size_offset)
        )
        ...
        self.bomb_y_offset = self.coin.get_height() + math.ceil(
            11 * self.game.settings.SCALE
        )
        ...
        self.one_quarter_heart = game.image_loader.get_stat_bar_image(
            "one_quarter_heart"
        )
        self.half_heart = game.image_loader.get_stat_bar_image("half_heart")
        self.three_quarters_heart = game.image_loader.get_stat_bar_image(
        ...
        self.bar_width = (
            self.game.image_loader.minimap["background"].get_width()
            - 2 * self.bar_x_offset
        )
        self.bar_height = int(
            self.full_bar_image.get_height() * self.game.settings.SCALE
        )
        self.bar_x = self.game.settings.WIN_WIDTH - self.bar_width - self.bar_x_offset
        self.bar_y = (
            self.game.image_loader.minimap["background"].get_height()
            + self.bar_y_offset
        )
        self.resize_bars()
        ...
        self.time_font = pygame.font.Font(
            self.font_path, int(self.STAT_BARS_HEALTH_SIZE)
        )
    ...
    def resize_bars(self):
        self.full_bar_image = pygame.transform.scale(
            self.full_bar_image, (self.bar_width, self.bar_height)
        )
        self.empty_bar_image = pygame.transform.scale(
            self.empty_bar_image, (self.bar_width, self.bar_height)
        )
    ...
    def calculate_hearts_cntrs(self):
        ...
        self.is_fractional_heart = int(
            health > self.floor_health + 0.1 and health < self.ceil_health - 0.1
        )
        if health <= 0:
            self.is_fractional_heart = False

        self.full_hearts_cntr = max(self.floor_health, 0) + int(
            health - self.floor_health >= 0.9
        )
    ...
    def draw_bomb(self, screen):
        ...
        screen.blit(
            text, (self.coin_x + self.TILE_SIZE // 3, self.coin_y + self.bomb_y_offset)
        )

    def draw_progress(self, screen):
        width = (
            self.player.rooms_cleared
            / self.MAX_ROOMS_TO_CLEAR
            * self.full_bar_image.get_width()
        )
        height = self.full_bar_image.get_height()
        screen.blit(self.empty_bar_image, (self.bar_x, self.bar_y))
        screen.blit(
            self.full_bar_image, (self.bar_x, self.bar_y), (0, 0, width, height)
        )
