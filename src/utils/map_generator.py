...
class MapGenerator:
    ...
    def create_map_scheme():
        ...
        while not satisfied:
            ...
            distance_array = [
                [-inf for _ in range(MAP_RANGE)] for _ in range(MAP_RANGE)
            ]
            ...
            satisfied, key_rooms = MapGenerator._check_if_map_valid(
                distance_array, row_max, col_max
            )
    ...
    def _check_if_map_valid(arr, row_, col_):
        ...
        key_rooms = {"start": None, "shop": None, "boss": None}
        ...
        for row in range(len(arr)):
            for col in range(len(arr[row])):
                if arr[row][col] == 0:
                    ...
                    arr[row][col] >= furthest_distance // 2
                    and arr[row][col] <= furthest_distance - 1
                ):
                    neighbors = 0
                    for i in range(4):
                        new_row = row + d_row[i]
        ...
        if (
            key_rooms["start"] is None
            or key_rooms["shop"] is None
            or key_rooms["boss"] is None
        ):
            return False, key_rooms
